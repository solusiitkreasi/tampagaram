@foreach ($popups as $popup)
    ...
    @if ($type == 1)
        <div data-popup_delay="{{$popup->delay}}" data-popup_id="{{$popup->id}}" id="modal-popup{{$popup->id}}" class="popup-wrapper">
            <div>
                <img class="lazy" data-src="{{asset('assets/img/popups/' . $popup->image)}}" alt="Popup Image" width="100%">
            </div>
        </div>
    @elseif ($type == 2)
        <div data-popup_delay="{{$popup->delay}}" data-popup_id="{{$popup->id}}" id="modal-popup{{$popup->id}}" class="popup-wrapper">
            ...
            <div class="popup-two bg_cover lazy" data-bg="{{asset('assets/img/popups/' . $popup->background_image)}}">
                <div class="popup_main-content">
                    ...
                    <div class="subscribe-form">
                        <form id="popupSubscribe{{$popup->id}}" class="subscribeForm" action="{{route('subscribe')}}" method="POST">
                            @csrf
